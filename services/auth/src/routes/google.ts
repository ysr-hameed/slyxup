...
route.get("/google", async (c) => {
  const redirectUri = c.env.GOOGLE_CALLBACK_URL;
...
const callbackDef = createRoute({
  ...
  responses: {
    200: {
      content: {
        "application/json": {
          schema: apiResponseSchema(z.object({ token: z.string(), jwt: z.string(), user: z.any() })),
        },
      },
...
route.openapi(callbackDef, async (c) => {
  ...
  if (!tokenResponse.ok) {
    logger.error("google_token_exchange_failed", { status: tokenResponse.status });
    return c.json({ success: false, error: "Failed to exchange code" }, 400);
  }
  ...
  const existing = await db.select().from(authSchema.oauthAccounts).where(
    and(eq(authSchema.oauthAccounts.provider, "google"), eq(authSchema.oauthAccounts.providerUserId, googleUser.id)),
  ).get();
  ...
  if (existing) {
    ...
    await db.insert(authSchema.oauthAccounts).values({
      id: generateId(), userId, provider: "google",
      providerUserId: googleUser.id, createdAt: now,
    }).run();
  }
  ...
  await db.insert(authSchema.sessions).values({
    id: sessionId, userId, token, expiresAt, createdAt: new Date().toISOString(),
  }).run();

  const jwt = await signToken(
    { sub: userId, email: googleUser.email, platform_id: "" },
  ...
  return c.json({
    ...
    data: {
      token, jwt,
      user: { id: userId, email: googleUser.email, name: googleUser.name, avatarUrl: googleUser.picture },
    },
  });
});
