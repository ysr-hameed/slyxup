      ...
      content: {
        "application/json": {
          schema: apiResponseSchema(z.object({
            ...
            avatarUrl: z.string().nullable(), emailVerified: z.boolean(), hasPassword: z.boolean(), createdAt: z.string(),
          })),
        },
      },
...
route.openapi(routeDef, async (c) => {
  ...
  return c.json({
    success: true,
    data: { id: user.id, email: user.email, name: user.name, avatarUrl: user.avatarUrl, emailVerified: !!user.emailVerified, hasPassword: !!user.passwordHash, createdAt: user.createdAt },
  });
});
