...
export function createSlyxupClient(config: SlyxupClientConfig): SlyxupClient {
  return {
    ...
    admin: createAdminClient({ baseUrl: baseUrl(config, "adminBaseUrl", "https://admin.slyxup.online"), apiKey: config.apiKey }),
    notification: createNotificationClient({ baseUrl: baseUrl(config, "notificationBaseUrl", "https://notification.slyxup.online"), apiKey: config.apiKey }),
  };
}
